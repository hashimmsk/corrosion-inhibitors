"""
Train on extended features (6 + 40 RDKit) WITHOUT VSG.

Baseline for comparing with train_extended_vsg.py.
"""
...
CV_SPLITS = 5
N_ITER = 50
OUT_DIR = Path(__file__).resolve().parent / "data" / "models" / "extended"
...
def main():
    if not (EXTENDED_DIR / "train.csv").exists():
        raise FileNotFoundError("Run: python3 preprocess_extended.py")
    ...
    print(f"Training on extended features: {len(extended_cols)} (6 original + 40 RDKit)")
    ...
    print("\nRefitting on TRAIN+VAL, evaluating on TEST...")
    for r in results:
    ...
    report = {
        ...
        "all_models": [
            {"model": r["model"], "val_metrics": r["val_metrics"], "test_metrics": r["test_metrics"], "best_params": r["best_params"]}
            for r in results
        ],
    }
    ...
    print("\n" + "=" * 60)
    print("Extended (no VSG) Training Complete")
    print("=" * 60)
    print(f"Best model: {best['model']}, Test R²: {best['test_metrics']['r2']:.4f}")
