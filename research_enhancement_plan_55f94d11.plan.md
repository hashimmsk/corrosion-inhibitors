---
name: Research Enhancement Plan
overview: A comprehensive plan to significantly improve the corrosion inhibitors ML project by incorporating techniques, methods, and presentation strategies extracted from the four contextual research papers -- addressing the core issue of R^2 = 0.417 through multiple complementary pathways.
todos:
  - id: poly-features
    content: Implement 2nd-degree polynomial feature engineering (C, Mw, HLB, EO, Conc, pH interactions) with aggressive pruning to top 4-5 terms
    status: pending
  - id: medium-encoding
    content: "Test 4 medium/pH encoding strategies: one-hot, drop pH, mixed-effects, medium x feature interactions"
    status: pending
  - id: expanded-models
    content: Add XGBoost, GBM, LightGBM, GPR, ElasticNet, Lasso, Ridge, PLS, AdaBoost, Bagging, ExtraTrees, KNN to model arsenal
    status: pending
  - id: kde-vsg
    content: Implement KDE-based Virtual Sample Generation on training set only, with medium-aware augmentation and bandwidth tuning
    status: pending
  - id: feature-selection
    content: Build 5-algorithm ensemble feature selection pipeline (Pearson, MI, F-test, RFE, PLS VIP) with composite ranking
    status: pending
  - id: sobol-analysis
    content: Run Sobol global sensitivity analysis using SALib, with/without pH, per medium
    status: pending
  - id: cv-upgrade
    content: Upgrade to 5-fold stratified CV by medium + nested CV for unbiased evaluation
    status: pending
  - id: outlier-detection
    content: Apply 4 outlier detection algorithms (IsolationForest, LOF, OneClassSVM, EllipticEnvelope) with PCA visualization
    status: pending
  - id: uncertainty
    content: Add uncertainty quantification via GPR, quantile RF, and conformal prediction (MAPIE)
    status: pending
  - id: pub-figures
    content: "Generate publication-quality figures: methodology flowchart, Sobol bars, model comparison heatmap, PCA outlier plot, learning curves"
    status: pending
  - id: hierarchical-model
    content: "Implement multilayer model: molecular features -> latent layer -> IE, following Paper 4's framework"
    status: pending
  - id: full-benchmark
    content: "Run complete benchmark: 23 models x with/without VSG x with/without polynomial features x per medium -- create comprehensive results table"
    status: pending
isProject: false
---
